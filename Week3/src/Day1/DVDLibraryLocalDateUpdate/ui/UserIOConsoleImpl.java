...
public class UserIOConsoleImpl  implements UserIO{
    ...
    @Override
    public LocalDate printDate(LocalDate date) {
        return date;
    }

    public String getUserReleaseDate(String prompt) {
        System.out.println(prompt);
        String releaseDate = null;

        try{
            releaseDate = LocalDate.parse(scanner.nextLine(), DateTimeFormatter.ofPattern("MM/dd/yyyy")).toString();

        }catch (Exception e) {
            System.out.println("Invalid pattern for date, please use date format: mm/dd/yyyy");
            getUserReleaseDate(prompt);
        }
