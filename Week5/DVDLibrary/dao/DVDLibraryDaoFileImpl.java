...
public class DVDLibraryDaoFileImpl implements DVDLibraryDAO{
    ...
    @Override
    public DVDLibrary updateReleaseDate(String dvdTitle, String releaseDateStr) throws DVDLibraryDaoException {
        loadRoster();
        DVDLibrary title = dvds.get(dvdTitle);
        LocalDate releaseDate = LocalDate.parse(releaseDateStr, DateTimeFormatter.ofPattern("MM/dd/yyyy"));
    ...
    private DVDLibrary unmarshallDVDLibrary(String dvdAsText){
        ...
        StringBuilder sbUserDate = new StringBuilder(dvdTokens[1]);
        LocalDate localDate = LocalDate.parse(sbUserDate);

        String userDate = localDate.format(formatters);
        LocalDate parsedDate = LocalDate.parse(userDate, formatters);
        // Index 1 - releaseDate
        dvdFromFile.setReleaseDate(parsedDate);
