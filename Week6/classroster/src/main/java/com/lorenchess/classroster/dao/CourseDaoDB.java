...
public class CourseDaoDB implements CourseDao{
    ...
    /**
     * We wrap the code of the getCourseById method in a try-catch in case the Course does
     * not exist.
     * We start with a SELECT query to get the basic Course object.
     * We follow up by calling the getTeacherForCourse method, where we JOIN from Teacher
     * to Course to get a Teacher object.
     * We then call the getStudentsForCourse method, where we JOIN from Student to course_student
     * to get a list of Student objects for this Course.
     * If we catch an exception in getCourseById, we return null because it means the Course
     * does not exist.
     * */
     ...
     * We pass the list of Courses into associateTeacherAndStudents, where we loop through
     * the list and call our existing Teacher and Students methods to fill in the data for each Course.
     * */
